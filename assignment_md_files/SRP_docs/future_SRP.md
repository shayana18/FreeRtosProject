...
- Add a stronger production-style recovery policy for WCET overruns inside SRP critical sections. The current kernel cleanup releases SRP bookkeeping state so the scheduler does not deadlock, but it does not guarantee that the interrupted application-level critical section completed safely.
