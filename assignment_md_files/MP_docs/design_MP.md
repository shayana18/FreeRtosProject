...
For online partition placement, this implementation uses best-fit rather than first-fit. The kernel admits tasks one at a time during `xTaskCreate()`, so an offline heuristic like first-fit decreasing was not a natural match. Between the two simple online choices, best-fit was preferred because with only two cores it adds almost no extra cost, but it reduces scan-order bias and tends to preserve room for future larger-utilization tasks. For example, if the current core utilizations are `0.2` and `0.6` and a new task has utilization `0.3`, then first-fit would place it on the first core and produce `0.5` and `0.6`, while best-fit places it on the second core and produces `0.2` and `0.9`. The best-fit result leaves one core largely free, which makes later admission of a larger task more likely.
